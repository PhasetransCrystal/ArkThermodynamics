...
public class ContainerHelper {
    ...
    public static void handleSlotClick(int actionIndex, int slotIndex, Player player, AbstractContainerMenu menu) {
                ...
                if (slot.isActive()) {
                    if (slot.mayPlace(carried)) {//如果允许放入
                        if ((actionIndex & 0b1) == 1) {//左键全部放入
                            ItemStack mark = carried.copy();
                            newCarried = slot.safeInsert(carried);
                            if (ItemStack.isSameItemSameComponents(newCarried,mark)) {//如果没有成功放入（即物品不一样），交换物品
                                ItemStack took = slot.safeTake(slot.getItem().getCount(), Integer.MAX_VALUE, player);
                                slot.safeInsert(newCarried);
                                newCarried = took;
                            }
                        } else if (((actionIndex & 0b10) >> 1) == 1) {//右键放入一个
                            newCarried = slot.safeInsert(carried, 1);
                        }
                    } else if (ItemStack.isSameItemSameComponents(carried, item) && slot.mayPickup(player) && (actionIndex & 0b1) == 1) {//在不允许放入时，尝试取出
